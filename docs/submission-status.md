...
- Hosting: the existing chatgpt.site release is unchanged and still serves React. A Python-capable host/account is needed to publish Streamlit; see `deployment.md`. The local Python app runs without an application login.
- Model: default Python mode uses exact source excerpts and makes no provider calls. The optional model adapter is covered with fake-provider tests; no new live-provider benchmark is claimed.

Nothing has been submitted to the academy form. Review the evidence, create the Google Doc and final live recording, then submit the links yourself.
